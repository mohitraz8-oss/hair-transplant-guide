// Video Call — one function handles submit, status and delete (action field).
...
module.exports = async (req, res) => {
  ...
  if (action === 'delete') {
    ...
    try {
      const look = await fetch(supaUrl + '/rest/v1/video_calls?select=purchase_token&access_token=eq.' + encodeURIComponent(bookingToken), { headers });
      ...
      const del = await fetch(supaUrl + '/rest/v1/video_calls?access_token=eq.' + encodeURIComponent(bookingToken), { method: 'DELETE', headers: Object.assign({ Prefer: 'return=minimal' }, headers) });
  ...
  const preferred_time = (body.preferred_time || '').toString().slice(0, 120);
